...
async function sendToBraze(payload) {
  const dryRun = String(process.env.BRAZE_DRY_RUN || '').toLowerCase() === 'true';
  const endpoint = (process.env.BRAZE_REST_ENDPOINT || '').replace(/\/$/, '');
  const apiKey = process.env.BRAZE_REST_API_KEY || '';
  if (dryRun) return { dry_run: true, braze_sent: false, braze_response: null };
  if (!endpoint) throw new Error('Missing BRAZE_REST_ENDPOINT environment variable.');
  if (!apiKey) throw new Error('Missing BRAZE_REST_API_KEY environment variable.');
